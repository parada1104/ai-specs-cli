...
## Residual D1 correction evidence (verifier-mandated, feat/go-hook-gate @ 904add9)

**Residual D1 (BLOCKING, independent verifier of 904add9)**: `materialize_hook_script` routed `output is None` (no verified binary, crash, timeout, malformed stdout) DIRECTLY to `_python_materialize_hook_script`, bypassing `_fallback_materialize_and_reconcile`. A fake Go binary that wrote the rendered gate and then crashed/returned garbage/timed out left the gate on disk with no lock baseline and returned success with `hook skipped (no provenance)` — the next sync would leave it untracked forever.

**Correction**: the dispatcher now snapshots the pre-Go destination state (`None` = absent) before invoking Go and routes `output is None` through the SAME `_fallback_materialize_and_reconcile` path used for wrote-null/mismatched records, with `announce=False` — the reconciliation note is emitted as a plain warning WITHOUT the token, so an infrastructure-failure run still carries exactly one `GO_HOOK_GATE_BRIDGE_FALLBACK` warning (from `go_materialize_hook`). Reconciliation contract: on-disk bytes exactly `_render_hook_content(...)` → baseline recorded with sha256 of the ON-DISK bytes; not provably CLI-rendered AND changed from the pre-Go snapshot → warn, then FAIL CLOSED (`RuntimeError`) instead of silently leaving an untracked gate; unchanged → historical preserve behavior (user-modified / no provenance), never seeded or overwritten. Symlink refusal still precedes any read/write. The fail-closed guard applies to all three reconcile callers (wrote-null and mismatched records included).

- RED (pre-implementation, 32-test suite): 3 errors (D1-C garbage-stdout, D1-E exit-70, D1-D timeout — no lock baseline after a real Go write) + 1 failure (changed unproven bytes did not fail closed); the preexisting-user regression passed on the old code, as expected (Go never writes there); all 27 prior tests unaffected.
- GREEN commands and observed results (all on feat/go-hook-gate):
  - `python3 -m unittest tests.test_hook_gate_bridge` — Ran 32 tests, OK.
  - `WORKTREE_GATE_BIN=$PWD/dist/worktree-gate-current python3 -m unittest tests.test_hook_gate_bridge` — Ran 32 tests, OK.
  - `python3 -m unittest tests.test_materialize_bridge tests.test_template_actuator_bridge tests.test_tracker_card_gate_hook tests.test_trello_mcp_workflow_recipe` — Ran 102 tests, OK.
  - `/tmp/verify-hook-gate/harness.py` (independent verifier harness, read-only vs the worktree) — TOTAL: 19 checks, 0 FAIL; D1-C/D1-D/D1-E PASS (baseline = on-disk digest), D1-F/D1-G unchanged PASS (D1-G now also fails closed).
- New tests: `HookGateRepairTests.test_malformed_stdout_after_go_write_reconciles_the_lock`, `test_crash_exit_70_after_go_write_reconciles_the_lock`, `test_timeout_after_go_write_reconciles_the_lock` (timeout constant patched to 1s), `test_infra_failure_with_preexisting_user_gate_preserves_bytes_and_baseline`, `test_infra_failure_leaving_unproven_changed_bytes_fails_closed`; all stubs really write the destination before failing. No existing assertion weakened.
