...
- **Harness env layout for direnv**: wizard writes `ai-specs/.env`, generates
  `ai-specs/.env.example`, and ensures a merge-safe managed block in project-root
  `.envrc` (`dotenv_if_exists` for app `.env` + `ai-specs/.env`). Migrates legacy
  `ai-specs/.envrc` exports. Doctor WARNs for missing direnv, managed block, or
  empty harness keys.

### Fixed
- **direnv allow path mismatch**: secrets no longer land only in `ai-specs/.envrc`
