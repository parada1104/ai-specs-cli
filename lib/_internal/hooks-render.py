...
EVENT_MAP: dict[str, dict[str, Any]] = {
    "pre-tool-use": {
        ...
        "pi": "tool_call",
        "omp": "tool_call",   # omp is a pi fork; same event model
    },
    "post-tool-use": {
        "claude": "PostToolUse",
...
# TS import path for the omp (Oh My Pi / can1357/oh-my-pi) ExtensionAPI.
# Verified June 2026: omp's official examples import from @oh-my-pi/pi-coding-agent,
# NOT from @earendil-works/pi-coding-agent (that is the upstream pi package).
OMP_EXT_IMPORT = "@oh-my-pi/pi-coding-agent"
...
def _module_script_decl(script_path: str) -> str:
    """Generated `const SCRIPT` bound to the adapter's module location.

    Every TS adapter (.opencode/plugin, .pi/extensions, .omp/extensions)
    sits exactly two levels below the project root, so `../../` + script_path
    resolves beside the materialized launcher at runtime — no relative SCRIPT
    and no sync-time machine-specific absolute path.
    """
    return f"const SCRIPT = fileURLToPath(new URL(\"../../{script_path}\", import.meta.url));\n"
...
def render_opencode(hook: dict[str, Any], native_event: str, project_root: Path) -> None:
    ...
    content = (
        ...
        "// The materialized launcher resolves beside this generated module at\n"
        "// runtime, so relocating the plugin keeps working from any process cwd.\n"
        + _module_script_decl(hook["script_path"])
        + f"const MATCHER = {json.dumps(hook.get('matcher') or '')};\n"
        "const ENV = {\n"
        f"{env_lines}"
        "};\n\n"
        "// One deterministic value drives both the event cwd and the child\n"
        "// process cwd: a string, absolute, existing directory wins after outer\n"
        "// whitespace is trimmed; anything else falls back to the process cwd.\n"
        "const normalizeDirectory = (raw) => {\n"
        "  if (typeof raw !== \"string\") return process.cwd();\n"
        "  const trimmed = raw.trim();\n"
        "  if (trimmed === \"\" || !isAbsolute(trimmed)) return process.cwd();\n"
        "  let isDir = false;\n"
        "  try { isDir = statSync(trimmed).isDirectory(); } catch { isDir = false; }\n"
        "  if (!isDir) return process.cwd();\n"
        "  return trimmed;\n"
        "};\n\n"
        ...
        "      if (MATCHER) {\n"
        "        // Case-insensitive: OpenCode tool ids may be lowercase while\n"
        "        // recipe matchers use Claude-style names (Write, Edit, Bash).\n"
        "        const re = new RegExp(`^(?:${MATCHER})$`, \"i\");\n"
        "        if (!re.test(toolName)) return;\n"
        "      }\n"
        "      const cwd = normalizeDirectory(directory);\n"
        "      const event = {\n"
        "        event: \"pre-tool-use\",\n"
        "        tool_name: toolName,\n"
        ...
        "        });\n"
        "      } catch {\n"
        "        return; // thrown child-process exception: fail open\n"
        "      }\n"
        "      if (res?.status !== 2) {\n"
        "        // non-2 exit, child spawn error, or missing status: fail open\n"
        "        return;\n"
        "      }\n"
...
def render_pi(hook: dict[str, Any], native_event: str, project_root: Path) -> None:
    ...
    content = (
        ...
        "import { fileURLToPath } from \"node:url\";\n\n"
        "// The materialized launcher resolves beside this generated module at\n"
        "// runtime; the pi event context stays process cwd (no workspace root\n"
        ...
        "    if (MATCHER) {\n"
        "      // Case-insensitive: pi/omp tool names are lowercase (write, edit)\n"
        "      // while matchers use Claude-style names (Write, Edit).\n"
        "      const re = new RegExp(`^(?:${MATCHER})$`, \"i\");\n"
        "      if (!re.test(toolName)) return;\n"
        "    }\n"
        "    // pi/omp tool input uses `path`; normalize to `file_path` so hook\n"
        "    // scripts written against the Claude event shape keep working.\n"
        "    const rawInput = call?.input ?? call?.arguments ?? {};\n"
        "    const event = {\n"
        "      event: \"pre-tool-use\",\n"
        "      tool_name: toolName,\n"
        "      tool_input: { ...rawInput, file_path: rawInput.file_path ?? rawInput.path ?? rawInput.notebook_path },\n"
        "      cwd: process.cwd(),\n"
        "    };\n"
...
# --- Omp (TS extension, decision via return {block:true}) -------------------

def render_omp(hook: dict[str, Any], native_event: str, project_root: Path) -> None:
    base = _shim_basename(hook)
    path = project_root / ".omp" / "extensions" / f"{base}.ts"
    env = hook.get("env") or {}
    env_lines = "".join(
        f"  {json.dumps(k)}: {json.dumps(str(v))},\n" for k, v in sorted(env.items())
    )
    content = (
        "// GENERATED by ai-specs — do not edit.\n"
        f"// Runtime hook {hook['recipe']}:{hook['id']} (event {hook['event']}).\n"
        f"import type {{ ExtensionAPI }} from \"{OMP_EXT_IMPORT}\";\n"
        "import { spawnSync } from \"node:child_process\";\n"
        "import { fileURLToPath } from \"node:url\";\n\n"
        "// The materialized launcher resolves beside this generated module at\n"
        "// runtime; the omp event context stays process cwd (no workspace root\n"
        "// is claimed by this adapter).\n"
        + _module_script_decl(hook["script_path"])
        + f"const MATCHER = {json.dumps(hook.get('matcher') or '')};\n"
        "const ENV: Record<string, string> = {\n"
...
def render(resolved_hooks_path: Path, agent: str, project_root: Path) -> list[str]:
    ...
    for hook in hooks:
        ...
        if agent == "claude":
            ...
            render_omp(hook, native_event, project_root)
        else:
            warnings.append(_warn(
