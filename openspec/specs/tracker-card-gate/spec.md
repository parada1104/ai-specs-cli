# tracker-card-gate Specification

## Purpose

Define the `trello-mcp-workflow` portable `pre-tool-use` tracker-card gate hook: its
activation predicate, production-path enforcement modes, `openspec/**` exemption,
fail-open behavior, optional shell-mode coverage, and the dogfood default.
...
The gate SHALL be active only when **all** hold:
...
Production-path writes SHALL be enforced by mode when the gate is active and a
matched file-write tool targets a path under the configured production directory set
(default includes at least `lib`, `catalog`, and `bin`; exact set locked in design /
config), and the active non-archive change lacks both a valid `## Tracker` link
section and a `tracker.none` exemption:
...
Writes under `openspec/**` MUST never be blocked by this gate, in any mode,
including `openspec/changes/**` planning files and the `## Tracker` section itself.
Agents MUST be able to create the link artifact and planning files without deadlock.
...
Malformed stdin JSON, missing path/command fields, and unexpected lookup errors MUST
fail open (exit `0`). A buggy guard must never wedge all editing.
...
The recipe MAY additionally distribute a sibling shell hook that SHALL invoke the
same `hooks/tracker-card-gate.sh` (dual-hook pattern: distinct hook id, matcher
covering `Bash|Shell|Execute|Terminal`) so Cursor's file-write matcher skip rule
cannot swallow shell coverage.
