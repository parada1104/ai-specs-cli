...
Implemented in the catalog by `catalog/recipes/playwright-ui-flow` and
`catalog/recipes/playwright-mcp`, which share a single canonical discipline
skill.
...
If design ships more than one recipe for this capability family, the recipes MUST
share discipline through a **single canonical discipline skill** (one
...
Whatever topology design locks (1 configurable recipe, 2 siblings, or 3), each shipped recipe MUST include:
