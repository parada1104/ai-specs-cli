...
Define how `session-bootstrap` resolves session focus: consult the bound `tracker`
capability for new or ambiguous changes, degrade gracefully when it is unavailable,
and keep tracker consultation optional when no tracker capability is bound.

## Requirements

### Requirement: Mandatory tracker consult when tracker is bound

`session-bootstrap` (session-context recipe) SHALL be capability-agnostic and MUST
consult the bound **tracker** capability for **new** or **ambiguous** changes / focus
resolution when a project has a **tracker** capability bound (auto-bound or
explicitly bound to a provider such as `trello-mcp-workflow`).
