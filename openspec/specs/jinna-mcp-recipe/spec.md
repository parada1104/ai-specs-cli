# jinna-mcp-recipe Specification

## Purpose

Define how the `jinna-mcp-recipe` catalog recipe declares, acquires, verifies, and materializes the
local OpenProject MCP provider for AI runtimes. Requirement text uses RFC 2119 terminology, and the
scenario examples are normative acceptance examples. In this specification `provider` means the Go
project `parada1104/jinna-provider`; `provider binary` means its local executable `jinna`;
`OpenProject` means the remote Self-Hosted service; and `MCP host` means an AI runtime that launches
the local stdio server. Provider readiness is an external prerequisite for Apply/Verify, not a reason
to weaken these requirements.

## Requirements
### Requirement: Declare the provider dependency
...
### Requirement: Detect and reuse the provider
...
### Requirement: Offer an explicit GitHub Release installation
...
### Requirement: Select and verify the provider release
...
### Requirement: Validate archive contents and install atomically
...
### Requirement: Resolve the provider command for MCP materialization
...
### Requirement: Configure OpenProject safely
...
### Requirement: Preserve protocol and operational boundaries
...
### Requirement: Preserve compatibility and explain recovery
...
### Requirement: Evidence and provider readiness gate
...
### Requirement: Validate declared environment values
