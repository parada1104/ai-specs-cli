# config-wizard-help Specification

## Purpose

Define recipe-configuration help behavior: valid questionary prompting for MCP
environment variables, curated how-to-get guidance for known env vars, `boolean`
type normalization, and `help_text` exposure for catalog config fields.

## Requirements

### Requirement: MCP env-var prompting uses valid questionary API

Secret MCP environment variables SHALL be prompted with `questionary.password` (or
`questionary.text` with `is_password=True`) when `ai-specs configure-recipes` (or
recipe-add / init) prompts for MCP environment variables collected from enabled
recipes. The prompt SHALL NOT pass a `password=` keyword argument to
`questionary.text`.
