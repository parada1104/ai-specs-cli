# recipe-cli-deps Specification

## Purpose

Define how recipe `[[deps.cli]]` binaries are checked and optionally installed:
detection stays non-destructive by default; TTY sessions may opt in to brew/apt
install from a constrained static map; `direnv` may be offered on the env scaffold
path.
