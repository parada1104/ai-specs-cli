...
- THEN the rules MUST include a post-merge base-sync rule referencing `git pull --ff-only` and `{config.base_branch}`
...
- THEN it MUST pull from the same resolved remote (not a hardcoded remote that contradicts its push remote)
