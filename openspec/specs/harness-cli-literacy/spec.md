...
Literacy skills MUST NOT introduce a per-project `ai-specs/bin/ai-specs` shim as the
delivery mechanism. They MAY include a one-line path-resolution footnote using
...
Referenced `ai-specs <subcommand>` invocations in the three literacy skills MUST
correspond to subcommands advertised by `bin/ai-specs` help (or aliases
