...
NEW capability. `ai-specs/.ai-specs.lock` SHALL record only CLI provenance —
`[meta]` with `cli_version` and `synced_at`. It SHALL NOT contain per-file
content hashes for skills or recipes. The lock is the CLI-provenance signal that
travels with a fresh clone (the machine-local cache `meta.toml` does not).
