...
### Requirement: Canonical Tracker link section

When the `trello-mcp-workflow` recipe is enabled, every active (non-archive)
OpenSpec change that is linked to a Trello card SHALL persist that link as a
`## Tracker` section inside the change's `proposal.md` (fallback: `tasks.md`
for tasks-only changes). This section is the sole card-link contract for
enforcement surfaces (skill, doctor, gate, evals). No new artifact file per
change: the link lives in the SDD artifacts the project already reads. The
capability MUST NOT treat a folder-schema field named `trello_card_id` as a
real required schema key; any skill vocabulary that still says
`trello_card_id` SHALL mean "the `card_id` recorded in the `## Tracker`
section".

A valid `## Tracker` section SHALL be a tolerant bold-key (or plain
...
mode) SHALL be declared in `openspec/config.yaml` under `tracking:`.

#### Scenario: Linking writes the Tracker section with required keys
...
- **THEN** it MUST use the `## Tracker` section of the change's `proposal.md`
  (fallback `tasks.md`)
...
enforcement that would otherwise require a valid `## Tracker` section.
Archives are grandfathered and are out of scope for this exemption contract.

#### Scenario: tracker.none allows change without a Tracker section

- **GIVEN** the `trello-mcp-workflow` recipe is enabled
- **AND** active change `<slug>` has `openspec/changes/<slug>/tracker.none`
- **AND** the change's `proposal.md` lacks a `## Tracker` section
- **WHEN** card-link enforcement evaluates the change
- **THEN** the missing link section MUST NOT be treated as a contract violation
...
- **AND** the agent MUST either create/link a card and write the `## Tracker`
  section, or write the `tracker.none` exemption marker with a logged reason
...
| **Missing** `## Tracker` section (and no `tracker.none`) while recipe + marker are active | enforceable by doctor WARN and by the tracker-card gate under `warn`/`always` |
...
- **AND** it MUST NOT write a fabricated `## Tracker` section
...
- **AND** active change `<slug>` has neither a valid `## Tracker` section nor
  `tracker.none`
...
that, when the recipe is enabled, agents establish a valid `## Tracker` link
section (or an explicit `tracker:none` exemption) for the active change before
apply-time production work. Brief text MUST name the canonical contract: the
`## Tracker` section of the change's `proposal.md` (fallback `tasks.md`).

#### Scenario: Brief names Tracker section precondition

- **GIVEN** the catalog `trello-mcp-workflow` recipe after this change
- **WHEN** `[provides.brief].workflow_rules` is read
- **THEN** the rules MUST require a link section (or `tracker:none`) before
  apply-time production work
- **AND** MUST name the `## Tracker` section as the canonical contract
...
persist the link via the `## Tracker` section.
...
#### Scenario: Card creation from bundled template writes the Tracker section
...
- **AND** the capability SHALL write a valid `## Tracker` section in the
