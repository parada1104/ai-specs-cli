...
- [x] **T1** — RED: unit tests for archive/artifact pre-merge helper
...
- [x] **T6** — RED: smoke tests for `setup_runtime_skills`, model defaults,
      natural-prompt guard (prompt must not contain `/plan`), scenario loader
      with `mode = plan|build`.
- [x] **T7** — GREEN: multi-runtime harness (`claude`/`opencode`/`pi`/`omp`),
      `EVALS_RUNTIME` / `EVALS_MODEL`, plan-mode invocation, NDJSON/text
      parsing as needed.
- [x] **T8** — Rewrite AC3 scenario: natural prompt + seeded app + Standard
      assertions (`tasks.md` + `specs/**`); keep forbidden production globs.
- [x] **T9** — Add stub scenarios AC4 (build after auth) / AC5 (archive) as
