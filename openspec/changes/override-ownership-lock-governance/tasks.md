...
- [ ] **T1.1** — Extend `tests/test_lock.py` (or sibling) for `[managed.*]`
...
- [ ] **T1.2** `[P]` — RED classifier cases: missing, managed_current,
...
- [ ] **T1.3** `[P]` — RED `materialize_template`: managed_stale + `auto`
...
- [ ] **T1.4** `[P]` — RED recipe_schema accepts `update_policy` values and
  rejects unknown.
  **Req:** recipe-schema delta.

- [ ] **T1.5** `[P]` — RED doctor messaging aligned with classifier
  (user-modified vs managed-stale under confirm/never-force).
  **Req:** doctor.

- [ ] **T1.6** — RED explicit refresh: delete target + sync reseeds + lock.
  **Req:** explicit refresh.

- [ ] **T1.7** — Confirm Phase 1 RED evidence (failures for missing behavior,
...
- [ ] **T2.1** — Implement `[managed.*]` in `lib/_internal/lock.py` (load/write,
  header comment). Preserve `[meta]` / `[agents.*]`; keep dropping skill hashes.
  **Done when:** T1.1 green.

- [ ] **T2.2** — Implement shared classifier (util or small module); render-aware
...
- [ ] **T3.1** — Wire `materialize_template` decision tree to classifier +
  policies; record lock on CLI write; keep hook always-overwrite regression.
  **Done when:** T1.3 + hook regression green.

- [ ] **T3.2** — Update `doctor._check_stale_template_overrides` to shared
  classifier.
  **Done when:** T1.5 green.

- [ ] **T3.3** — Parse optional `update_policy` on `TemplateRef` in
  `recipe_schema.py`.
  **Done when:** T1.4 green.

- [ ] **T3.4** — Explicit refresh path (document `rm`+sync; optional
...
- [ ] **T4.1** — Document auto / confirm / never-force + hooks always-CLI in
  appropriate recipe/docs surface (trello + worktree-flow README or central
  sync docs).
- [ ] **T4.2** — Promote deltas into canonical
  `openspec/specs/{sync-lock,override-ownership,recipe-schema,worktree-flow}/`.
- [ ] **T4.3** — Verify DocRef `condition` behavior; wire or explicitly exclude
...
- [ ] **T5.1** — Focused unit tests for lock/classifier/materialize/doctor/schema.
- [ ] **T5.2** — Full `./tests/run.sh` and `./tests/validate.sh` green.
- [ ] **T5.3** — Record verify evidence; ready for PR only after authorization
...
**READY_FOR_AUTH: yes** — full chain written; awaiting human go-ahead before apply.
