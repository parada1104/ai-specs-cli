...
class SyncMarkerPreservationTests(unittest.TestCase):
    ...
    def test_sync_without_marker_preserves_untracked_agents_md(self):
        """A divergent pre-existing brief is preserved even without the marker.

        The renderer cannot prove that a no-baseline file was generated by ai-specs.
        It therefore preserves divergent bytes and reports the explicit adoption and
        permanent marker remedies rather than inferring ownership from its shape.
        """
        target = self._make_target()
