...
class TrackerCardGateHookTests(unittest.TestCase):
    ...
    def test_shell_preprocessing_matches_bash_truth_matrix(self):
        ...
        cases = (
            ...
            ("fold-space-backslash-comment-continuation", "echo foo \\\n#bar \\\ngh pr create --fill", 2),
            ("fold-space-backslash-comment-continuation-archive", "echo foo \\\n#bar \\\nopenspec archive needs-card", 2),
            ...
            ("backslash-dq-escape-preserved", "cat > docs/x <<\"EO\\OF\"\nbody\nEO\\OF\ngh pr create --fill", 2),
            ("backslash-dq-escape-literal", "cat > docs/x <<\"EO\\OF\"\nbody\nEOOF\ngh pr create --fill", 0),
            ...
            ("bare-amp-inline-archive", "git add -A & openspec archive needs-card", 2),
            ("mv-midcmd-redirect-archive", "mv openspec/changes/needs-card 2>&1 openspec/changes/archive/", 2),
            ("aispecs-midcmd-redirect-archive", "ai-specs change 2>&1 archive needs-card", 2),
            ("mv-trailing-redirect-archive", "mv openspec/changes/needs-card openspec/changes/archive/ 2>&1", 2),
            ...
            ("coproc-named-archive", "coproc mine { openspec archive needs-card; }", 2),
            ...
            ("dollar-bracket-shift-midword-echo", "echo a$[1<<2]\ngh pr create --fill", 2),
            ("dollar-bracket-shift", "echo $[1<<2]\nopenspec archive needs-card", 2),
            ("same-unit-unbalanced-double", "gh pr create --title \"My PR --body x", 0),
            ("same-unit-unbalanced-apostrophe", "gh pr create --title don't", 0),
            ("same-unit-unbalanced-single", "gh pr create --fill '", 0),
            ("same-unit-unbalanced-archive", "openspec archive needs-card 'oops", 0),
            ("earlier-line-unbalanced-quote", "echo start\ngh pr create --title 'unterminated", 0),
