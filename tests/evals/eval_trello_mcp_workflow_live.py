"""Live trello-mcp-workflow recipe evals. Requires EVALS_LIVE=1."""
...
LIVE_SCENARIOS: tuple[str, ...] = (
    ...
    "ac_missing_card_gate_no_bash_skip",
...
def _seed_change(
    ...
    *,
    ...
    card_id: str = "6a6ebd5e2cd9a2fcd419e62c",
) -> None:
    d = root / "openspec" / "changes" / slug
    d.mkdir(parents=True, exist_ok=True)
    body = f"# {slug}\n\n## Intent\n\nDemo change for evals.\n"
    if with_tracker:
        body += (
            "\n## Tracker\n\n"
            f"- **card_id**: `{card_id}`\n"
            "- **url**: https://trello.com/c/WHZ3fLzD/56-demo\n"
        )
    (d / "proposal.md").write_text(body)


def _prepare_fixture(root: Path, fixture: str) -> None:
    if fixture in ("", "empty_active"):
        return
    if fixture == "deficient_change":
        _seed_change(root, "missing-card-demo", with_tracker=False)
        marker = root / ".recipe" / "trello-mcp-workflow" / "bootstrap-ready"
        marker.parent.mkdir(parents=True, exist_ok=True)
        marker.write_text("ready\n")
        return
    if fixture == "carded_change":
        _seed_change(root, "phase-demo", with_tracker=True)
        return
    if fixture == "retro_deficient":
        _seed_change(root, "retro-demo", with_tracker=False)
        return
    raise ValueError(f"unknown trello-mcp-workflow fixture: {fixture}")
...
class TrelloMcpWorkflowLiveEvals(unittest.TestCase):
    def _run_scenario(self, scenario_id: str, runtime: str):
        ...
        if meta.get("wire_hooks") is True:
    ...
    def test_trello_mcp_workflow_scenarios(self):
        selected = _selected_scenarios()
        self.assertTrue(selected, "no trello-mcp-workflow scenarios selected")
